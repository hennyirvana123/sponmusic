    ...
    def process(self, source, directory, report):
        ...
        try:
            ...
            subprocess.run(['ffmpeg', '-nostdin', '-v', 'error', '-protocol_whitelist', 'file,pipe',
                            '-i', source, '-t', '61', '-vn', '-ac', '1', '-ar', str(sample_rate),
                            '-y', decoded], check=True, capture_output=True, timeout=60)
            audio, rate = sf.read(decoded, dtype='float32')
            duration = len(audio) / rate
            if duration < .25 or duration > 60 or not np.isfinite(audio).all():
                raise ValueError('Input must be valid audio between 0.25 and 60 seconds')
            report['input_duration_seconds'] = duration
            log.info('piano faithful input duration %.3fs', duration)
            stage = 'model_load'
            report['stage'] = stage
            if self.model is None:
                # Upstream resolves/downloads its default note + pedal checkpoint.
                # Explicit path permits an externally verified, immutable checkpoint.
                self.model = PianoTranscription(device='cpu', checkpoint_path=os.environ.get('PIANO_AMT_CHECKPOINT') or None)
            ...
            log.info('piano faithful inference finished %.3fs', report['inference_duration_seconds'])
            ...
            pedals = sum(msg.type == 'control_change' and msg.control == 64 for track in midi.tracks for msg in track)
            if not notes:
            ...
            # Validation-only cleanup: identical bytes preserve all model events,
            # including note velocities, timings and CC64. No arrangement is applied.
            with open(os.path.join(directory, 'cleaned.mid'), 'wb') as file:
                file.write(data)
            report.update(stage='completed', output_note_count=notes, pedal_event_count=pedals,
                          cleanup='validation_only_byte_identical')
            log.info('piano faithful output notes=%d pedal_events=%d', notes, pedals)
        except Exception:
