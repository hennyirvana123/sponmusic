...
export default function Index() {
  ...
  useEffect(()=>{const file=location.state?.midiFile;if(!(file instanceof File))return;let cancelled=false;readMidi(file,location.state?.aiResult?0:12).then(data=>{if(cancelled)return;if(location.state?.aiResult){const p={name:data.name,bpm:data.bpm,notes:data.tracks.flatMap(t=>t.notes),transpose:0};if(!validProject(p))throw Error('MIDI model melewati batas proyek.');setProject(p);setLoop(false);setMidiSource(null);}else setMidiImport(data);}).catch(e=>{if(!cancelled)toast.error(e instanceof Error?e.message:'MIDI gagal dibaca');});return()=>{cancelled=true;};},[location.key]);
